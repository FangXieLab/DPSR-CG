...
def get_scatter_transform_l(dataset):
    shape = SHAPES[dataset]
    base_scattering = Scattering2D(J=2, shape=shape[:2])
    class ScaledScattering(nn.Module):
    ...
    scattering = ScaledScattering(base_scattering)
    K = 81 * shape[2]
    (h, w) = shape[:2]
    return scattering, K, (h//4, w//4)
